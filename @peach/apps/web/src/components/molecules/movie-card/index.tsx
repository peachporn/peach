    ...
    >
      ...
      <div className='size-full rounded-xl bg-[var(--color)] transition-all group-hover:rounded-b-none'>
        <Link href={href ?? "#"}>
          <Image
            alt={title}
            className={cn("size-full rounded-xl bg-[var(--color)] object-cover transition-all", {
          ...
          {loadPreview && (
            <VideoPreview
              movieId={id}
              className={cn(
                "absolute left-0 top-0 h-full w-full overflow-hidden rounded-xl opacity-0 transition-opacity",
                {
                  "opacity-100": videoPlaying,
                }
              )}
              onVideoLoaded={() => setVideoPlaying(true)}
            />
          )}
        </Link>
      </div>
